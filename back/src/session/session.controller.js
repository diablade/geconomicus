...
SessionController.start = async (req, res, next) => {
	try {
		...
		await EventService.postNow(DB_EVENTS.SESSION_STARTED, sessionUpdated._id, null, PLAYER_TYPE.MASTER, '-', {
			gamesRules_idx: sessionUpdated.gamesRules,
		}); //.map(rule => rule.idx)}
		socket.emitAckTo(ROOMS.session(req.body.sessionId), IO.SESSION.STARTED, {
			gamesRules: sessionUpdated.gamesRules,
		});
		return res.status(200).json(sessionUpdated);
	} catch (err) {
		log.error(`[SessionControler] start error:`, err);
...
SessionController.delete = async (req, res, next) => {
	try {
		...
		if (
			(env.environment === 'production' && bcrypt.compareSync(password, env.admin_password)) ||
			(env.environment !== 'production' && password === 'admin')
		) {
