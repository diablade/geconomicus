...
describe('AVATAR controller', () => {
    ...
    describe("AVATAR CREATE", () => {
        test('should create avatar and emit socket event', async () => {
            ...
            expect(socket.emitTo).toHaveBeenCalledWith(
                session._id,
                expect.stringContaining(C.NEW_AVATAR),
                expect.objectContaining({ name: 'test-avatar-name', id: avatarId, })
            );
        });
    });
    describe("AVATAR UPDATE", () => {
        test("should update avatar successfully", async () => {
            ...
            expect(socket.emitTo).toHaveBeenCalledWith(
                session._id,
                expect.stringContaining(C.UPDATED_AVATAR),
    ...
    describe("AVATAR DELETE", () => {
        test("should delete avatar successfully", async () => {
            ...
            expect(socket.emitTo).toHaveBeenCalledWith(
                session._id,
                expect.stringContaining(C.DELETED_AVATAR),
                expect.objectContaining(
                    {
                        avatarId: avatarId,
                    }
