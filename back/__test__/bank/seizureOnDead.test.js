...
jest.mock('./../../src/player/player.service.js');
...
describe('seizureOnDead', () => {
	...
	const cards = [
		{_id: card1, weight: 1, price: 1},
		{_id: card2, weight: 2, price: 2},
		{_id: card3, weight: 1, price: 1},
		{_id: card4, weight: 2, price: 2}
	];
	...
	const mockCredits = [
		{_id: credit1, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer, status: C.RUNNING_CREDIT},
		{_id: credit2, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer, status: C.RUNNING_CREDIT}
	];
	const mockCredits2 = [
		{_id: credit1, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer2, status: C.RUNNING_CREDIT},
		{_id: credit2, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer2, status: C.RUNNING_CREDIT}
	];
	const mockCredits3 = [
		{_id: credit1, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer3, status: C.RUNNING_CREDIT},
		{_id: credit2, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer3, status: C.RUNNING_CREDIT}
	];
	const mockCredits4 = [
		{_id: credit1, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer4, status: C.RUNNING_CREDIT},
		{_id: credit2, interest: 1, amount: 3, idGame: mockIdGame, idPlayer: mockIdPlayer4, status: C.RUNNING_CREDIT}
	];
	const mockGame = {
		_id: mockIdGame,
		typeMoney: C.DEBT,
	...
	it('should handle a player with sufficient coins to pay all debts', async () => {
		...
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			1, {_id: mockIdGame, 'credits._id': credit1},
			{$set: {'credits.$.status': C.CREDIT_DONE, 'credits.$.endDate': expect.any(Number)}});
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			2, {_id: mockIdGame, 'credits._id': credit2},
			{$set: {'credits.$.status': C.CREDIT_DONE, 'credits.$.endDate': expect.any(Number)}});
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			3, {_id: mockIdGame, 'players._id': mockIdPlayer},
			{
				$inc: {
	...
	it('should handle a player with some coins but sufficient cards to pay the rest of debts', async () => {
		...
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			...
			{$set: {'credits.$.status': C.CREDIT_DONE, 'credits.$.endDate': expect.any(Number)}});
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			3, {_id: mockIdGame, 'players._id': mockIdPlayer2},
			{
				$inc: {
		...
		expect(decksService.pushCardsInDecks).toHaveBeenCalledTimes(1);
		expect(decksService.pushCardsInDecks).toHaveBeenNthCalledWith(
			1, mockIdGame, [
				{_id: card4, weight: 2, price: 2},
				{_id: card3, weight: 1, price: 1},
			]);
	...
	it('should handle a player with no coins and sufficient cards to pay all debts', async () => {
		...
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			...
			{$set: {'credits.$.status': C.CREDIT_DONE, 'credits.$.endDate': expect.any(Number)}});
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			3, {_id: mockIdGame, 'players._id': mockIdPlayer3},
			{
				$inc: {
		...
		expect(decksService.pushCardsInDecks).toHaveBeenCalledTimes(1);
		expect(decksService.pushCardsInDecks).toHaveBeenNthCalledWith(
			1, mockIdGame, expect.arrayContaining(cards));
	});
	...
	it('should handle a player with no coins and insufficient cards to pay all debts', async () => {
		...
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			...
			{$set: {'credits.$.status': C.CREDIT_DONE, 'credits.$.endDate': expect.any(Number)}});
		expect(GameModel.updateOne).toHaveBeenNthCalledWith(
			3, {_id: mockIdGame, 'players._id': mockIdPlayer4},
			{
				$inc: {
					'players.$.coins': -0, // Total interest and amount credit as possible
					'bankInterestEarned': 0,
		...
		expect(decksService.pushCardsInDecks).toHaveBeenCalledTimes(1);
		expect(decksService.pushCardsInDecks).toHaveBeenNthCalledWith(
			1, mockIdGame, expect.arrayContaining([
				{_id: card1, weight: 1, price: 1},
				{_id: card2, weight: 2, price: 2},
			]));
	});
