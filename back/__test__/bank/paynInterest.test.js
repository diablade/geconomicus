...
import payInterest from '../../src/bank/bank.service';
import getCreditOnActionPayment from '../../src/bank/bank.service';
...
import * as C from '../../src/config/constantes';
...
describe('payInterest', () => {
    ...
    it('should successfully pay interest and return updated credit', async () => {
      ...
      expect(BankTimerManager.addTimer).toHaveBeenCalledWith(
        'credit123',
        true,
        mockGame.timerCredit,
        mockCreditUpdated
      );
    });

    it('should throw an error if coins are insufficient', async () => {
        getCreditOnActionPayment.mockResolvedValue({ credit: mockCredit, canPay: false });

      await expect(
        payInterest('credit123', 'game456', 'player789')
      ).rejects.toThrow('Not enough coins to pay interest.');
    });
  });
