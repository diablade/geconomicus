...
- **Both warnings are overlays, not snackbars.** A snackbar competes with the board and is easy to miss on a phone mid-trade; a full-screen panel is not. The Halfway Nudge therefore drops the `{{remaining}}` figure it used to carry — the point is "you are past half", and the credit panel already shows the exact time. Escalation is carried by icon, colour and sound (⏰ amber `nudge` → ⚠️ flashing red `high_alarm`), not by wording.
