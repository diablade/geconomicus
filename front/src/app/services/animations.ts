...
export const animations = [
	trigger('list', [transition(':enter', [query('@items', [animateChild()], { optional: true })])]),
	trigger('item', [
		transition(':enter', [
			style({ transform: 'translateY(-100rem)' }),
			animate('600ms ease-out', style({ transform: 'translateY(0)' })),
		]),
		transition(':leave', [animate('600ms ease-in', style({ transform: 'translateY(-100rem)' }))]),
	]),
	trigger('coinFlip', [
		transition('void => *', []),
		transition('* => *', [
	...
	trigger('prisonDoor', [
		transition(':enter', [
			...
			animate('2000ms', style({ transform: 'translateX(0rem)' })),
		]),
		transition(':leave', [
			style({ transform: 'translateX(0rem)' }),
			animate('2000ms', style({ transform: 'translateX(-100rem)' })),
		]),
	]),
	trigger('cardFlip', [
		state(
